...
class engine:
    ...
    def search(query):   
        ...
        r = requests.get(link)
        content = r.text        
        drug_names, descriptions = engine.find_drugs_in_content(query, content)                
        names_output, smiles_output = engine.drug_names_to_smiles(drug_names)        
        return names_output, smiles_output, descriptions
    
    def find_drugs_in_content(query, content):        
    ...
    def drug_names_to_smiles(names):    
        names_output = []
        smiles_output = []
        for name in names:
